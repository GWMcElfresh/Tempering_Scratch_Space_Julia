// Extended Flexible Dirichlet-Multinomial (EFDM) regression model
// Supports per-category mixing weights w_norm[d], enabling negative covariance
...
data{
	...
	vector<lower=0>[D] w_hyper; // hyperparameter for per-category Beta priors
}
...
parameters {
	...
	real<lower=0> aplus;              // overall precision
	simplex[D] p;                     // baseline Dirichlet weights
	vector<lower=0,upper=1>[D] w_norm; // per-category mixing weights
}
...
model {
	...
	// Likelihood (Extended Flexible Dirichlet-Multinomial)
	for (i in 1:N) {
		real temp;
		real temp2;
		vector[D] alpha;
		...
		for(j in 1:D){
			...
			temp    = temp + lgamma(alpha[j]+Y[i,j]) - lgamma(alpha[j]) - lgamma(Y[i,j]+1);
			Y_temp[j] = log(p[j]) + lgamma(aplus+tau[j]) - lgamma(aplus+n[i]+tau[j])
			            + lgamma(alpha[j]) + lgamma(alpha[j]+tau[j]+Y[i,j])
			            - lgamma(alpha[j]+tau[j]) - lgamma(alpha[j]+Y[i,j]);
		}
